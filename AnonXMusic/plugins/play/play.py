    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            if (
                ...
                return await mystic.edit_text(
                    "**-↲لا يستطيع هذا الروبوت تشغيل مسارات وقوائم التشغيل الخاصة بـ Spotify، يرجى الاتصال بالمالك الخاص بي واطلب منه إضافة مشغل Spotify ♪..**"
                )
            if "track" in url:
                try:
            ...
            try:
                ...
                await mystic.edit_text(
                    "**-↲يوجد خطأ في البوت، يرجى الإبلاغ عنه لدعم في أسرع وقت ممكن ♪..**"
                )
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    "**-↲يرجى تشغيل دردشة الصوتيه للاستمتعاع بالموسيقه ♪..**",
                )
            except Exception as e:
                return await mystic.edit_text(
...
async def anonymous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
            "**-↲أنت مسؤول مجهول ♪..**\n\n**-↲قم بالعودة إلى حساب المستخدم لاستخدامي ♪..**",
            show_alert=True,
        )
    except:
        return
