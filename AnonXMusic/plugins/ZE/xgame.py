...
async def handle_game_command(c, msg):
    chat_id = msg.chat.id
    if msg.text == "تفعيل الالعاب":
        if chat_id in azan_enabled_chats:
            await msg.reply_text("الالعاب مفعل بالفعل في هذه المجموعة")
        else:
            game_enabled_chats.append(chat_id)
            await msg.reply_text("تم تفعيل الالعاب بنجاح")
    elif msg.text == "تعطيل الالعاب":
        if chat_id in azan_enabled_chats:
            game_enabled_chats.remove(chat_id)
            await msg.reply_text("تم تعطيل الالعاب بنجاح")
        else:
            await msg.reply_text("الالعاب معطل بالفعل في هذه المجموعة")
